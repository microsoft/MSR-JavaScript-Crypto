...
export function toBase64(data: ArrayBuffer | number[] | ArrayBufferView, base64Url: boolean = false): string;
...
export function bytesToHexString(bytes: number[], separate: boolean = true): string;
...
export function getVector(length: number, fillValue: number = 0): number[];
